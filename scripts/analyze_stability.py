...
for ax, gait in zip(axes[0], ("trot", "walk")):
    ...
    ax.set(title=gait.title(), xlabel="Achieved-command gate: duty factor",
           ylabel=r"Orbital convergence $\chi$ (lower is better)")
...
rates = table.groupby(["gait", "command_df"], as_index=False).agg(
    command_gate_rate=("command_gate_rate", "mean"),
    orbit_gate_rate=("cycle_gate_rate", "mean"),
    finite_difference_gate_rate=("finite_difference_gate_rate", "mean"))
if rates.empty:
    invalid_message(ax)
else:
    ...
    x = range(len(rates))
    width = .25
    ax.bar([i-width for i in x], rates.command_gate_rate, width, label="Command")
    ax.bar(x, rates.orbit_gate_rate, width, label="Periodic orbit")
    ax.bar([i+width for i in x], rates.finite_difference_gate_rate, width,
