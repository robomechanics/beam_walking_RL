"""Train the independent flat-ground high-duty walk controller."""
...
parser.add_argument("--num_envs", type=int, default=HIGH_DUTY_WALK_TRAINING_NUM_ENVS)
parser.add_argument("--iterations", type=int, default=HIGH_DUTY_WALK_TRAINING_ITERATIONS)
parser.add_argument("--seed", type=int, default=HIGH_DUTY_WALK_TRAINING_SEED)
...
if args.mode == "train" and not args.development and (
        args.num_envs != HIGH_DUTY_WALK_TRAINING_NUM_ENVS
        or args.iterations != HIGH_DUTY_WALK_TRAINING_ITERATIONS
        or args.seed != HIGH_DUTY_WALK_TRAINING_SEED
        or abs(args.stance_start_probability - .10) > 1e-12):
    parser.error(
        "Primary high-duty walk training requires 3072 environments, 1800 updates, "
        "seed 4, and 0.10 grounded-start probability")
...
def snapshot_sources(output):
    ...
    for relative in HIGH_DUTY_WALK_TRAINING_SOURCE_FILES:
...
def smoke(env, wrapped):
    ...
    if env.action_manager.action.shape != (args.num_envs, 12):
        raise RuntimeError("HighDutyWalk controller must emit 12 joint actions")
    gait_command = command(env)
    sampled_strata = {
        (int(gait), round(float(duty), 3))
        for gait, duty in zip(gait_command.values[:, 4], gait_command.values[:, 1])
    }
    required = {(1, round(duty, 3)) for duty in HIGH_DUTY_WALK_LEVELS}
    if args.num_envs >= len(required) and not required <= sampled_strata:
        raise RuntimeError(
            f"Smoke reset did not cover high-duty walk gait/DF strata: {sampled_strata}")
    ...
    for step in range(max(args.steps, CYCLE_STEPS)):
        ...
        if not bool(torch.isfinite(observation["policy"]).all()
            ...
            raise RuntimeError("HighDutyWalk smoke produced a nonfinite value")
        ...
        if bool((changed & ~done & (expected != 0)).any()):
            raise RuntimeError("HighDutyWalk command changed away from a cycle boundary")
        if step == 0 and bool(gait_command.stance_start.any()):
            ...
            if (not bool((first_grounded_contacts == 4).all())
                ...
                raise RuntimeError("Grounded high-duty walk reset did not begin supported")
    ...
    if not bool(complete.any()):
        raise RuntimeError("HighDutyWalk smoke found no complete phase-locked cycle")
    return {
        "policy_observation_dimension": 68,
        "action_dimension": 12,
        "finite_observations_rewards": True,
        "phase_alignment_verified": True,
        "all_high_duty_walk_anchors_present": required <= sampled_strata,
...
def main():
    ...
    try:
        ...
        lineage = high_duty_walk_lineage_id(training_source_sha256, args.seed)
        provenance = {
            "schema": "high_duty_walk_training_v1",
            ...
            "gait": "walk",
            "walk_duty_range": [HIGH_DUTY_WALK_LEVELS[0],
                                HIGH_DUTY_WALK_LEVELS[-1]],
            "balanced_discrete_duty_levels": list(HIGH_DUTY_WALK_LEVELS),
            "fixed_period_s": .48,
            "fixed_period_control_ticks": HIGH_DUTY_WALK_PERIOD_TICKS,
            "minimum_requested_swing_control_ticks": 2,
            "minimum_requested_swing_s": .04,
        ...
        if args.mode == "smoke":
            ...
            print("HIGH_DUTY_WALK_DUTY_SMOKE_OK", json.dumps(result), flush=True)
            return
        ...
        print("HIGH_DUTY_WALK_TRAINING_COMPLETE", json.dumps(throughput), flush=True)
    finally:
