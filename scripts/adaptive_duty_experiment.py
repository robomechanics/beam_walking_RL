"""Train the separate flat-ground controller used by the duty selector."""
...
parser.add_argument("--num_envs", type=int, default=ADAPTIVE_TRAINING_NUM_ENVS)
parser.add_argument("--iterations", type=int, default=ADAPTIVE_TRAINING_ITERATIONS)
parser.add_argument("--seed", type=int, default=3)
parser.add_argument("--warm-start", type=Path,
                    help="Development-only 68D checkpoint initialization")
...
if args.warm_start is not None and args.mode != "train":
    parser.error("Warm-start is available only for development training")
if args.warm_start is not None:
    args.development = True
if args.mode == "train" and not args.development and (
        args.num_envs != ADAPTIVE_TRAINING_NUM_ENVS
        or args.iterations != ADAPTIVE_TRAINING_ITERATIONS
        or args.seed != 3
        or abs(args.stance_start_probability - .10) > 1e-12):
    parser.error(
        "Primary adaptive training requires 3072 environments, 1800 updates, "
        "seed 3, and 0.10 grounded-start probability")
...
def snapshot_sources(output):
    ...
    for relative in ADAPTIVE_TRAINING_SOURCE_FILES:
...
def smoke(env, wrapped):
    ...
    for step in range(max(args.steps, CYCLE_STEPS)):
        ...
        if not bool(torch.isfinite(observation["policy"]).all()
            ...
            raise RuntimeError("Adaptive smoke produced a nonfinite value")
        ...
        if bool((changed & ~done & (expected != 0)).any()):
            raise RuntimeError("Adaptive command changed away from a cycle boundary")
        if step == 0 and bool(gait_command.stance_start.any()):
            ...
            if (not bool((first_grounded_contacts == 4).all())
                ...
                raise RuntimeError("Grounded adaptive reset did not begin supported")
    ...
    if not bool(complete.any()):
        raise RuntimeError("Adaptive smoke found no complete phase-locked cycle")
    return {
        "policy_observation_dimension": 68,
        "action_dimension": 12,
        "finite_observations_rewards": True,
        "phase_alignment_verified": True,
        "all_walk_trot_df_anchor_strata_present": required <= sampled_strata,
...
def main():
    ...
    try:
        ...
        lineage = adaptive_lineage_id(training_source_sha256, args.seed)
        warm_start_sha256 = (
            hashlib.sha256(args.warm_start.read_bytes()).hexdigest()
            if args.warm_start else None)
        provenance = {
            "schema": "adaptive_duty_training_v1",
            ...
            "walk_and_trot_duty_range": [.50, .75],
            "balanced_discrete_duty_levels": [
                duty for gait, duty in ADAPTIVE_COMMAND_STRATA if gait == 0],
            "minimum_requested_swing_s": .10,
            "fresh_training": bool(args.mode == "train" and not args.warm_start),
            "primary_training_protocol": bool(
                args.mode == "train" and not args.development),
            "development_warm_start": bool(args.warm_start),
            "warm_start_checkpoint": (
                str(args.warm_start.resolve()) if args.warm_start else None),
            "warm_start_checkpoint_sha256": warm_start_sha256,
            "task_sha256": task_sha256,
            "training_source_sha256": training_source_sha256,
            "training_lineage_id": (
                lineage if args.mode == "train" and not args.warm_start else None),
        ...
        if args.mode == "smoke":
            ...
            print("ADAPTIVE_DUTY_SMOKE_OK", json.dumps(result), flush=True)
            return

        runner = OnPolicyRunner(
            wrapped, agent.to_dict(), log_dir=str(args.output), device=env.device)
        if args.warm_start:
            saved_parent = runner.load(
                str(args.warm_start), load_optimizer=False)
            compatible_hash = files_sha256(ROOT, BASE_COMPATIBLE_TASK_FILES)
            if (not saved_parent
                    or saved_parent.get("task_sha256") != compatible_hash):
                raise ValueError(
                    "Warm-start checkpoint is not the compatible 68D base task")
            # This is weight initialization for a new adaptive curriculum.
            # Optimizer state and old iteration/curriculum progress are excluded.
        ...
        print("ADAPTIVE_DUTY_TRAINING_COMPLETE", json.dumps(throughput), flush=True)
    finally:
